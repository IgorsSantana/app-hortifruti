# init_db.py (Versão com lógica de inserção corrigida)
import os
import sqlite3
import psycopg2
...
# ... (código de criação de tabelas inalterado) ...
SQL_TYPE = {
...
# --- LÓGICA PARA POPULAR AS TABELAS (CORRIGIDA) ---
...
# 1. Criar uma lista mestra de produtos únicos (por nome)
...
# 2. Inserir ou atualizar cada produto único UMA SÓ VEZ
print(f"Encontrados {len(all_products_by_name)} produtos únicos para inserir/atualizar...")
for nome, produto_info in all_products_by_name.items():
    ...
    if is_postgres:
        cur.execute("""
            INSERT INTO products (name, unidade_fracionada, codigo_interno) VALUES (%s, %s, %s) 
            ON CONFLICT (name) DO UPDATE SET 
            unidade_fracionada = EXCLUDED.unidade_fracionada, codigo_interno = EXCLUDED.codigo_interno;
...
# 3. Agora, criar os vínculos de disponibilidade
print("Criando vínculos de disponibilidade por dia...")
cur.execute("DELETE FROM product_availability;") # Limpa os vínculos antigos para garantir consistência
